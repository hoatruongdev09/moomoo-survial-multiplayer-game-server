...
class Bullet {
  ...
  updatePosition(deltaTime) {
    ...
    if (
      this.position.clone().sub(this.startPosition).sqrMagnitude() >=
  ...
  checkColliderWithNpc() {
    ...
    for (const n of this.npcView) {
      this.user.game.testCollisionCircle2Circle(
        this,
        n,
        (response, objectCollider) => this.onHitNpc(response, objectCollider, n)
      );
    }
  }
  ...
  checkColliderWithPlayer() {
    ...
    for (const p of this.playerView) {
      this.user.game.testCollisionCircle2Circle(
        ...
        p,
        (response, objectCollider) =>
        this.onHitPlayer(response, objectCollider, p)
      );
    }
  }
  onHitNpc(response, object, objectInfo) {
    this.isDestroy = this.user.game.bulletHitNpc(
      this.user.idGame,
      objectInfo.id,
      this.damage
    );
  ...
  onHitPlayer(response, object, objectInfo) {
    this.isDestroy = this.user.game.bulletHitPlayer(
      this.user.idGame,
      objectInfo.id,
      this.damage
    );
